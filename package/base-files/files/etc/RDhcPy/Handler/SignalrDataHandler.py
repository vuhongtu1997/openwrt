...
class SignalrDataHandler(IHandler):
    ...
    def __handler_log_signalr_response(self, item):
        entity = item[1]
        data = item[2]
        print(f"\033[0;103mCloud >> HC : {entity} :\n\033[0;30m{data}\n")
        self.__logger.info(f"\033[0;103mCloud >> HC : {entity} :\n\033[0;30m{data}\n")
        pass

    def __handler_log_signalr_command(self, item):
        entity = item[1]
        data = item[2]
        print(f"\033[0;102mCloud >> HC : {entity} :\n\033[0;30m{data}\n")
        self.__logger.info(f"\033[0;102mCloud >> HC : {entity} : {data}\n")
        try:
    ...
    def __handler_signalr_auto_update_command(self, item):
        entity = item[1]
        data = item[2]
        self.__logger.info(f"\033[0;102mSignalR Data - {entity} Entity: {data}\n")
        print(f"\033[0;102mCloud >> HC : {entity} :\n\033[0;30m{data}\n")
        msg = {"CMD": "AUTO_UPDATE"}
        try:
